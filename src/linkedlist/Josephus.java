...
public class Josephus {
     ...
     * @param count number of people in circle
     * @param start person to start at (note that this is **not** zero-indexed)
     * @param step size of step between people when counting who's next to kill themselves
     */
    ...
    /**
     * Eliminate all but the last remaining member of the list according to the start & stop obj vars
    ...
    /**
     * Advances to the next person to leave the circle (size defined by the step variable) & removes them
     * (atomic operation)
    ...
    int advance() {
        ...
        return deleted.map(Link::getiData).orElse(-1);
    }
